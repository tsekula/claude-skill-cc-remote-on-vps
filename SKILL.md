...
Scripts (paths are relative to this skill's folder; all are bash — on Windows
run them through Git Bash, which is what Claude Code's Bash tool uses):

- `scripts/install-cli.sh` — runs **on the user's machine** (macOS, Linux,
  Windows) to install or update `hcloud` / `doctl` and put it on PATH.
...
2. **Local machine check — do it yourself, don't ask.** Work out the OS
   (`uname -s`: `Linux`, `Darwin` = macOS, `MINGW*`/`MSYS*` = Windows via Git
   Bash) rather than asking the user. All of this skill's scripts are bash; on
   Windows, run them through your Bash tool (Git Bash). Also confirm `ssh -V`
   works (Windows: `references/ssh-keys.md` §0 if it doesn't).
   - **Windows with no bash at all** (you only have PowerShell, or
     `Get-Command bash` finds nothing / only `C:\Windows\System32\bash.exe`,
     which is WSL, not Git Bash): install Git for Windows yourself — tell the
     user it's the standard Git installer and that Windows may show one
     "allow changes?" prompt they should accept:
     ```powershell
     winget install --id Git.Git -e --source winget --accept-package-agreements --accept-source-agreements
     ```
     You don't need to restart anything: from PowerShell, run every script in
     this skill as `& "$env:ProgramFiles\Git\bin\bash.exe" scripts/<name>.sh …`
     for the rest of the session. (Git's bundled `ssh`, `scp` and `ssh-keygen`
     are in that same bash.) If `winget` itself is missing, send the user to
     https://git-scm.com/download/win and wait for them.
3. **Install or update the provider CLI — yourself.** Run:
   ```bash
   bash scripts/install-cli.sh hcloud     # or: doctl
   ```
   It detects the OS/CPU, checks whether the CLI exists and is current (same
   major.minor as the latest GitHub release), and if not installs/updates it
   with no admin rights: via Homebrew/Scoop when that already owns the copy,
   otherwise the official GitHub release, **SHA-256 verified**, into
   `%LOCALAPPDATA%\Programs\<tool>` (Windows) or `~/.local/bin` (macOS/Linux),
   and adds that folder to the user's PATH permanently. Tell the user in one
   plain sentence what you're installing and why ("the official Hetzner
   command-line tool, so I can create the server for you") — don't hand them
   install instructions to follow. Its **last line** is `CLI_PATH=<path>`:
   your shell's PATH predates the install, so for the rest of this session
   call the CLI by that absolute path (or prepend its folder to `PATH` in each
   command). The provisioning scripts find it on their own. Only if the script
   fails, fall back to the manual options in `<provider ref>` §1.
4. **CLI authenticated?** DigitalOcean: `doctl account get`. Hetzner:
   `hcloud server list`. If it errors, the user needs an API token
   (`<provider ref>` §2 — give them the click-by-click steps and the direct
   link) and then one command that **they run in their own terminal**, because
   it prompts for the token and your shell has no keyboard. **Do not ask the
   user to paste the token to you.** Give them the exact line for their OS, and
   say to open a **new** terminal window so it sees the updated PATH:
   - Windows (PowerShell / Terminal): `hcloud context create cc-vps` /
     `doctl auth init`. If "not recognized", use the full path from
     `CLI_PATH`, e.g.
     `& "$env:LOCALAPPDATA\Programs\hcloud\hcloud.exe" context create cc-vps`.
   - macOS / Linux (Terminal): the same `hcloud context create cc-vps` /
     `doctl auth init`, or `~/.local/bin/<tool> …`.
   Then re-run the check yourself — the token is stored in the CLI's config
   file, which your shell reads too. Env-var alternatives for CI:
   `DIGITALOCEAN_ACCESS_TOKEN` / `HCLOUD_TOKEN`.
...
5. **SSH key.** See the dedicated section below — this is where a first-time
   user most often gets stuck, so handle it patiently and offer to generate a
   key for them.
6. **SSH port and username.** Ask for the login name of the new sudo user
...
7. **Image.** Default `ubuntu-24-04-x64` (DigitalOcean) / `ubuntu-24.04`
...
|---|---|---|
| **Default — Claude Code + Remote Control (Step 6)** | **~4 GB, or ~2 GB + `--swap 2G` as a floor** | Claude Code idles ~0.5–1 GB; add MCP/language servers, `npm install`, test suites and 1 GB OOMs mid-task. A 2nd vCPU keeps the agent responsive during builds. |
...
scripts/provision-digitalocean.sh --name web-01 --region fra1 \
  --size s-2vcpu-4gb --image ubuntu-24-04-x64 --ssh-key ~/.ssh/id_ed25519_web-01.pub
...
ssh-keygen -R "$IP" >/dev/null 2>&1   # providers recycle IPs; drop any stale entry
SSHO=(-i "$KEY" -o StrictHostKeyChecking=accept-new -o BatchMode=yes)
scp "${SSHO[@]}" scripts/harden.sh root@"$IP":/root/harden.sh
ssh "${SSHO[@]}" root@"$IP" "bash /root/harden.sh --user tom --port 22 --swap 2G --pubkey \"$(cat "$KEY".pub)\""
```

`-i "$KEY"` tells `ssh`/`scp` which private key to use; skip it only if the key
has the default name (`~/.ssh/id_ed25519`). `StrictHostKeyChecking=accept-new`
records the brand-new server's host key on first contact instead of asking
"Are you sure you want to continue connecting?" — your shell has no keyboard,
so that prompt would just fail with "Host key verification failed". It still
refuses a key that *changes* later. `BatchMode=yes` makes any other prompt fail
fast instead of hanging. Use the same options for every `ssh`/`scp` you run in
Steps 5–6 (shell variables don't carry over between your commands, so
re-define `KEY`/`SSHO` each time). Pass `--swap` on servers under
...
Add `--fail2ban` only if the user wants it (mention it once as an optional
extra; don't make a first-timer decide). With key-only SSH there's nothing to
brute-force, so it mostly quiets the logs and slows bots.
...
> - **Security fixes install themselves.** Ubuntu's automatic security updates
>   are switched on (they usually are already — we make sure), so known holes
>   get patched without you logging in.
...
>
> Not done unless you ask: `fail2ban` (bans IPs that keep failing to log in —
> optional, since there's no password to guess), 2FA for SSH.
...
3. If `--swap` was given and no swap is active: creates `/swapfile`, enables
   ...
   Then makes sure `unattended-upgrades` is installed and enabled (security
   updates only, no automatic reboots), and with `--fail2ban` installs
   fail2ban with an `sshd` jail on the chosen port. Any apt install waits for
   cloud-init / the first-boot apt run to release the dpkg lock.
...
5. Writes `/etc/ssh/sshd_config.d/00-hardening.conf` with
   `PermitRootLogin no`, `PasswordAuthentication no`,
   `PubkeyAuthentication yes`, `KbdInteractiveAuthentication no`, and `Port`
   only if it isn't 22. The `00-` prefix matters: sshd keeps the *first* value
   it reads, and cloud images often ship `50-cloud-init.conf` with
   `PasswordAuthentication yes`.
6. Runs `sshd -t` to validate the config, then `sshd -T` to confirm the
   *effective* values match. If either fails, the script removes its drop-in
   and aborts **without** restarting `sshd`, so the working config stays live.
7. Restarts `ssh` (and, on Ubuntu 24.04, regenerates and restarts
   `ssh.socket`, which owns the listening port). Existing connections
   (including yours) are not dropped.
...
```bash
ssh "${SSHO[@]}" -p <port> <user>@<ip> "sudo whoami && echo OK"
```
...
1. **Sizing check.** Claude Code needs ~0.5–1 GB and each Remote Control server
...
3. **Install.** Copy `scripts/setup-claude-code.sh` to the box, run it as the
   ...
   installs Claude Code with Anthropic's native installer (a self-updating
   binary at `~/.local/bin/claude`, no Node.js needed; it migrates an old npm
   install if it finds one), `tmux`,
...
4. **Log in** (interactive — you can't do it for them, but you can relay). In
   ...
   When you drive it with `ssh <host> "…"` (a non-login shell), `~/.local/bin`
   may not be on PATH yet: call `~/.local/bin/claude` explicitly, e.g.
   `tmux new -d -s login -c ~/projects/<name> ~/.local/bin/claude`.
...
- what was hardened: root SSH disabled, password auth disabled, passwordless
  ...
  `--swap` was used; automatic security updates on; fail2ban if
  `--fail2ban` was used (give the Step 4 ELI5 version if the user is new to this)
...
Use this when the box is already a Claude Code box (Claude Code + the
...
3. **Ensure the tooling is on the box.** `add-rc-server.sh` needs
   ...
   (skips an already-installed Claude Code, migrating an old npm install) and just adds the template.
4. **Run it:**
   ```bash
...
- **`harden.sh` re-runs** are safe: it skips user creation if the user exists
  and overwrites only its own `00-hardening.conf` drop-in (removing the
  `99-hardening.conf` that older versions wrote).
